    ...
    public int[] update(Camera camera, int[] pixels) {
        ...
        for(int x=0; x<width; x++) {
            ...
            while (!hit) {
                ...
                //Ensure mapX and mapY are within bounds before accessing map
                if (mapX >= 0 && mapX < mapWidth && mapY >= 0 && mapY < mapHeight) {
                    //Check if ray has hit a wall
                    if (map[mapX][mapY] > 0) hit = true;
                } else {
                    // If the ray goes out of bounds, stop checking
                    break;
                }
            }
            //Calculate distance to the point of impact, if a wall was hit
            if (hit) {
                if (side == 0)
